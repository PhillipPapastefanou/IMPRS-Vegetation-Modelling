...
class Model:
    ...
    def run(self):
        for t in range(int(self.params.nyears * self.NDAYS_PER_YEAR )):
            ...
            # Update soil processes, i.e. soil water content
            self.soil.Update(precip, self.vegetation.transpiration)
            # Update vegetation processes, i.e. stomatal conductance, photosynthesis and growth
            self.vegetation.Update(self.soil.soil_water, vpd, sw_rad, co2, temp)
